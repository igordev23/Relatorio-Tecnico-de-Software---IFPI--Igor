...
A Figura~\ref{fig:templates} apresenta a página de gerenciamento de templates. Nela, o usuário pode criar novos modelos de certificado, visualizar a lista de templates existentes e personalizar a aparência de cada um (cores, fontes, bordas) através de um editor visual inline com pré-visualização em tempo real. Esta tela atende aos requisitos \textbf{RF008} (upload de templates), \textbf{RF009} (personalização), \textbf{RF010} (selecionar template da galeria), \textbf{RF012} (criação de campos dinâmicos) e \textbf{RF013} (salvar configurações de mapeamento).
...
A Figura~\ref{fig:certificados} exibe a lista de certificados emitidos, com campo de busca por nome, CPF ou código de verificação. Cada certificado exibe o status (ativo ou vencido), o curso, a carga horária e opções de ação: copiar código de verificação, editar validade, baixar PDF e editar dados do certificado. Esta tela atende aos requisitos \textbf{RF027} (reemissão mantendo histórico), \textbf{RF028} (download individual), \textbf{RF039} (lista completa de certificados) e \textbf{RF040} (filtros por curso, período e status).
...
A Figura~\ref{fig:envios} apresenta o histórico de envios de e-mail realizados pelo sistema. Cada registro exibe o destinatário, o nome do participante, a data do envio e o status (enviado, falhou ou pendente). Esta tela atende ao requisito \textbf{RF031} (registrar status de envio).
...
A Figura~\ref{fig:verificar} mostra a página pública de verificação de autenticidade de certificados. O usuário informa o CPF do participante e o código de verificação presente no certificado. O sistema consulta a base de dados e retorna o resultado: certificado válido, expirado ou não encontrado. Esta tela atende aos requisitos \textbf{RF033} (portal público de validação), \textbf{RF034} (validação via código único) e \textbf{RF036} (exibir informações do certificado).
